...
#define TESTDIR "/tmp/bcs115/mountdir"
...
int main(int argc, char **argv) {
	...
	int i, fd = 0, ret = 0;
	struct stat st;
	fd = creat(TESTDIR "/file", FILEPERM);
	printf("fd for test1: %d\n", fd);
	if (fd < 0) {
		perror("creat");
	...
	for (i = 0; i < ITERS; i++) {
		...
		char * str = "hello there my name is brian";
		memset(buf, str, strlen(str));

		if (write(fd, buf, strlen(str)) != strlen(str)) {
